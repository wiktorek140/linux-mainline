...
void a5xx_set_hwcg(struct msm_gpu *gpu, bool state)
{
	...
	if (adreno_is_a506(adreno_gpu)) {
		for (i = 0; i < ARRAY_SIZE(a50x_hwcg); i++)
			gpu_write(gpu, a50x_hwcg[i].offset,
				state ? a50x_hwcg[i].value : 0);
	} else {
...
static int a5xx_me_init(struct msm_gpu *gpu)
{
	...
	if (adreno_is_a530(adreno_gpu) || adreno_is_a506(adreno_gpu)) {
...
static int a5xx_hw_init(struct msm_gpu *gpu)
{
	...
	if (adreno_is_a510(adreno_gpu)) {
		...
		gpu_write(gpu, REG_A5XX_PC_DBG_ECO_CNTL,
			  (0x200 << 11 | 0x200 << 22));
	} else if (adreno_is_a506(adreno_gpu)) {
		gpu_write(gpu, REG_A5XX_CP_MEQ_THRESHOLDS, 0x20);
	...
	gpu_write(gpu, REG_A5XX_TPL1_MODE_CNTL, (adreno_is_a506(adreno_gpu) ? 1 : 2) << 7);
	gpu_write(gpu, REG_A5XX_RB_MODE_CNTL, (adreno_is_a506(adreno_gpu) ? 1 : 2) << 1);
	if (adreno_is_a540(adreno_gpu))
	...
	if (adreno_is_a530(adreno_gpu) || adreno_is_a510(adreno_gpu))
		...
		gpu_write(gpu, REG_A5XX_CP_PROTECT(17),
			ADRENO_PROTECT_RW(0x10000, 0x4000));
	...
	if (!adreno_is_a510(adreno_gpu) && !adreno_is_a506(adreno_gpu))
...
static int a5xx_pm_suspend(struct msm_gpu *gpu)
{
	...
	/* A510 and A50x has 3 XIN ports in VBIF */
	if (adreno_is_a510(adreno_gpu) || adreno_is_a506(adreno_gpu))
		mask = 0x7;
